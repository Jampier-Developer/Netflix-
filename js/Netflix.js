...
================================================================ */
var ENC_EMAIL = 'uEnGydL/PmzWxcDY0QTnY8MOAEhAx9kJA3g9hp0Hs7/cNvLAPIk0FfQCLItAofwo/gLfJ00H/A+fftKHnB7L';
var ENC_PASS  = 'a6DKI1vP56Vo8TbhcXJTQqBM7mE81Q7S5rmchwsXjr3DdLVqFabTdaUD';
var ENC_DATE  = 'toDUuUI11Q/HkBKjvuqhvQJ2KzL6WDgnRWiqxHqox2yoMEtJYkU=';
...
/* Helper SHA-256 para comparación segura del código de emergencia */
...
/* Hash SHA-256 del código de emergencia (moviejampier).
   El código real NUNCA aparece en el código fuente; solo se compara
   su hash con SHA-256 — igual que un sistema real de contraseñas. */
var EMERGENCY_HASH = 'be5e35ddfd0eb5bea32b1e78a4ac71afd0ed06610d74c9ddcca3e26c5711bdd1';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('logout-btn').addEventListener('click', function(){
        ...
        // Limpiar sesión y DOM
        sessionStorage.removeItem('nfx_sk');
        emailEl.textContent = '';
    ...
    (function initAccess(){
        ...
        var cdInt     = null;
        var emergencyReady = false;

        function setupEmergency() {
            var eToggle = document.getElementById('emergency-toggle');
            var eForm   = document.getElementById('emergency-form');
            var eInput  = document.getElementById('emg-input');
            var eUnlock = document.getElementById('emg-unlock-btn');
            var eError  = document.getElementById('emg-error');
            var eVis    = document.getElementById('emg-vis-btn');

            eToggle.addEventListener('click', function() {
                var open = eForm.classList.contains('show');
                eForm.classList.toggle('show', !open);
                eToggle.classList.toggle('open', !open);
                if (!open) setTimeout(function(){ eInput.focus(); }, 80);
            });

            eVis.addEventListener('click', function() {
                var isP = eInput.type === 'password';
                eInput.type = isP ? 'text' : 'password';
                eVis.querySelector('i').className = isP ? 'fas fa-eye-slash' : 'fas fa-eye';
            });

            async function tryEmergencyUnlock() {
                var h = await sha256hex(eInput.value);
                if (h === EMERGENCY_HASH) {
                    clearAttempts();
                    clearInterval(cdInt);
                    cdInt = null;
                    ...
                    eToggle.classList.remove('open');
                    setTimeout(function(){ inputEl.focus(); }, 200);
                    showToast('Bloqueo eliminado · Puedes intentar de nuevo', 'info', 3500);
                } else {
                    eInput.value = '';
                    eError.classList.add('show');
                    setTimeout(function(){ eError.classList.remove('show'); }, 2500);
                    eInput.focus();
                }
            }

            eUnlock.addEventListener('click', tryEmergencyUnlock);
            eInput.addEventListener('keydown', function(e) {
                if (e.key === 'Enter') tryEmergencyUnlock();
                eError.classList.remove('show');
            });
        }
        ...
        async function verify(){
            ...
            if (result) {
                clearAttempts();
                sessionStorage.setItem('nfx_sk', btoa(pw));
                inputEl.value = '';
                initVault(result);
        ...
        // Restaurar sesión si existe (recarga de página sin cerrar sesión)
        var sk = sessionStorage.getItem('nfx_sk');
        if (sk) {
            (async function() {
                try {
                    var result = await tryUnlock(atob(sk));
                    if (result) {
                        initVault(result);
                        screen.classList.add('hidden');
                        return;
